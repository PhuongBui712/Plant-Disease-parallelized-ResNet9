...
const Form = ({ setImage, setIsPending, setUrl, setColor, setError, setPredict, setAdvice, setTrashBinImage }) => {
  const uploadImage = async (image) => {
    ...
    try {
      const response = await axios.post(`http://localhost:5000/process_traffic_status`, formData, {
        headers: {
      ...
      setUrl(data.traffic_status);
      setPredict(data.traffic_status);
      setAdvice(data.traffic_status);
